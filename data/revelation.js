/**
 * BACKCHAIN - Bible Data: Revelation (KJV)
 */
...
window.BibleData = {
    ...
    getVerseCount: function() {
        return this.allVerseRefs.length;
    },

    getRandomVerse: function() {
        var refs = this.allVerseRefs;
        var ref = refs[Math.floor(Math.random() * refs.length)];
        return { reference: ref, text: this.verses[ref] };
    },

    /**
     * Get all verses in biblical order (for Zen mode)
     * Starting from 22:1 going to 22:21
     */
    getAllVersesInOrder: function() {
        var result = [];
        for (var i = 0; i < this.allVerseRefs.length; i++) {
            var ref = this.allVerseRefs[i];
            result.push({ reference: ref, text: this.verses[ref] });
        }
        return result;
    },

    /**
     * Get verses for classic backchaining
     * Run 1: [22:21]
     * Run 2: [22:20, 22:21]
     * Run 3: [22:19, 22:20, 22:21]
     */
    getVersesForRun: function(runNumber) {
        var result = [];
        var totalVerses = this.allVerseRefs.length;
        var count = Math.min(runNumber, totalVerses);
        
        var startIndex = totalVerses - count;
        
        for (var i = 0; i < count; i++) {
            var verseIndex = startIndex + i;
            var ref = this.allVerseRefs[verseIndex];
            result.push({ reference: ref, text: this.verses[ref] });
        }
...
console.log('BibleData module loaded with ' + BibleData.getVerseCount() + ' verses');